...
using _37_WebApp_SQLite.Utilities;// il namespace per accedere alla cartella utilities e i file che ci sono dentro.
...
public class DeleteModel : PageModel
{
    ...
    public IActionResult OnGet(int id)//Carica i dati del prodotto con l'id specificato.
    {
        try
        {
            ...
            var Prodotti = DbUtils.ExecuteReader( "SELECT p.Id, p.Nome, p.Prezzo, c.Nome, f.Nome as CategoriaNome FROM Prodotti p LEFT JOIN Categorie c ON p.CategoriaId = c.Id LEFT JOIN Fornitori f ON p.FornitoreId = f.Id WHERE p.Id = @id",
            ...
            //dobbiamo identificare il prodotto separato dal reader che va a leggere la query, 
