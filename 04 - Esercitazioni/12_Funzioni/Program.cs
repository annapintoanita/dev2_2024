...
//puo essere void anche se restituisce dei valori perchè abbiamo usato i parametri out
...
int[] NumeriPari(int n) // n sta per il numero di elementi dell'array
{
    ...
    return numeriPari;//restituisce l'array
}
int[]numeri = NumeriPari(10);//utilizzo della funzione
foreach (int numero in numeri)
{
    Console.WriteLine(numero);// stampa 0,2,4,6,8
    //stampo con delay di un secondo
    System.Threading.Thread.Sleep(300); // stampa a ritmo più lento o più veloce in base all'indicazione
}

//esempio di funzione che restituisce array di stringhe
...
string[] ParolePari (string[]parole)
{
    ...
    return parolePari.ToArray();// restituisce la lista convertita in array
}
...
//in questo caso viene restituita una lista contenente la versione abbreviata di una serie di nomi 
List<string> NomiAbbreviati(List<string> nomi)
{
    ...
    foreach(string nome in nomi)
    {
        ...
        string abbreviato = $"{parti[0]} {parti[1][0]}";//creo una stringa col nome abbreviato
        nomiAbbreviati.Add(abbreviato);//aggiungo il nome abbreviato alla lista
    }
    return nomiAbbreviati;//restituisce la lista
}
...
//esempio di funzione che restituisce una lista 
//in questo caso viene restituita una lista contenente la versione abbreviata di una serie di nomi
//l'utente può specificare quanti caratteri vuole visualizzare
List<string> NomiAbbreviatiConInput(List<string> nomi, int lunghezza)
{
    ...
    {
        string[] parti = nome.Split(' ');//divido il nome in parti separate dallo spazio
        string abbreviato = $"{parti[0]} {parti[1].Substring(0, lunghezza)}.";//creo una stringa col nome abbreviato
        nomiAbbreviati.Add(abbreviato);//aggiungo il nome abbreviato alla lista
    }
    return nomiAbbreviati;//restituisce la lista
}
...
Dictionary<string, int> dizionario = new Dictionary<string, int> { { "uno", 1 }, { "due",2 } , { "tre", 3 }, { "quattro", 4 } };// dichiaro un dizionario di
...
Dictionary<string,int> valori = ValoriSpecifici(dizionario, chiavi);//utilizzo della funzione in modo da caricare un dizionario con i valori specifici
...
int Doppio(int n)
{
    return n * 2;// restituisce il doppio di n
                // il primo return che viene passato alla funzione Quadruplo
}
int Quarduplo(int n)
{
